...
_SRC_SKIP_DIRS  = {"__pycache__", ".git", "node_modules", ".venv", "venv", "dist", "build"}

# Maximum characters loaded from the source tree for a single review
_SRC_MAX_CHARS = 40_000
...
class DocumentQAAgent(BaseAgent):
    ...
    async def on_startup(self) -> None:
        ...
        for name, desc, inv, tags in self._OWN_TOOLS:
            await self.memory.register_tool(name, desc, "document_qa", inv, tags, "document_qa")
        log.info(
            "document_qa.startup",
    ...
    async def _handle_pdf_read(self, task: str) -> str:
        ...
        messages = [
            ...
            HumanMessage(content=f"Extracted PDF content:\n{truncate_file(pdf_content, 'pdf')}\n\nTask: {task}"),
        ]
        response = await self.llm_invoke(messages)
    ...
    async def _handle_architecture_review(self, task: str) -> str:
        ...
        messages = [
            ...
            HumanMessage(
                content=(
                    ...
                    "3. Memory architecture (Redis short-term, Postgres long-term, context_snapshots)\n"
        ...
        compile_result = await self._extract_and_compile_latex(arch_text, "architecture")
    ...
    async def _handle_latex_generate(self, task: str) -> str:
        ...
        ctx = ""
        if any(kw in task.lower() for kw in ("stack", "agent", "system", "architecture")):
            ctx = self._load_source_tree(max_chars=20_000)
        ...
        compile_result = await self._extract_and_compile_latex(latex_text, "generated_doc")
    ...
    async def _compile_latex(self, tex_path: Path) -> str:
        ...
        try:
            ...
            if code == 0 and pdf_path.exists():
                ...
                log.warning("document_qa.compile_failed", code=code, stderr=err_snippet[:200])
    ...
    def _load_pdfs_as_text(self) -> str:
        ...
        for path in self.docs_path.glob("*.pdf"):
            try:
                ...
                log.debug("document_qa.loaded_pdf", file=path.name, pages=len(reader.pages))
            except Exception as exc:
    ...
    async def _upload_to_drive(self, pdf_path: Path) -> None:
        ...
        if not folder_id or not creds_path or not Path(creds_path).exists():
            log.debug("document_qa.drive_skip",
        ...
        try:
            ...
            log.warning("document_qa.drive_libs_missing",
        ...
        try:
            ...
            await loop.run_in_executor(
                ...
                lambda: service.files().create(
                    body={"name": dated_name, "parents": [folder_id], "mimeType": "application/pdf"},
                    media_body=media,
                    fields="id",
                ).execute(),
            )
            ...
            await self._drive_upsert(service, folder_id, "architecture_latest.pdf", pdf_path)
